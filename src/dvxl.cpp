...
using namespace DVXL;

DVXL_GRID::DVXL_GRID(std::string filename, double legacy_resolution)
{
...
DVXL_GRID::DVXL_GRID(double Xdim, double Ydim, double Zdim, double resolution, float default_deformability, float default_sensitivity, OpenRAVE::Transform origin)
{
...
void DVXL_GRID::LoadLegacyFiletype(std::string filename, double legacy_resolution)
{
...
void DVXL_GRID::LoadCurrentFiletype(std::string filename)
{
    FILE *array_file = fopen(filename.c_str(), "r");
    if (array_file == NULL)
    {
...
void DVXL_GRID::SetValues(float deformability, float sensitivity, uint8_t R, uint8_t G, uint8_t B, uint8_t A)
{
    dvxl_t default_dvxl;
...
bool DVXL_GRID::WriteLegacyFiletype(std::string filename)
{
...
bool DVXL_GRID::WriteCurrentFiletype(std::string filename)
{
    FILE *array_file = fopen(filename.c_str(), "w");
    //Write file header
    int xDim = grid_.getNumCells(grid_.DIM_X);
...
bool DVXL_GRID::ConvertToLegacyFiletype(std::string filename)
{
    return WriteLegacyFiletype(filename);
}

bool DVXL_GRID::Save(std::string filename)
{
    return WriteCurrentFiletype(filename);
}

std::vector<float> DVXL_GRID::GetDefaults()
{
...
void DVXL_GRID::SetNewDefaults(float deformability, float sensitivity)
{
    default_deformability_ = deformability;
    default_sensitivity_ = sensitivity;
}

void DVXL_GRID::Reset()
{
    SetValues(default_deformability_, default_sensitivity_, 0, 0, 0, 0);
}
